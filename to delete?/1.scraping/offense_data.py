...
def ReformatDataFrame(year, dataframe, flag, merge, stat):
    ...
    if ((merge == 1) and (int(year) > 2012)):
        if int(year) == 2013:
            result.to_csv('C:/Users/Matt/Desktop/offense_Data.csv', index = False, sep=',', header = True)
            result = pd.DataFrame
        else:
            with open('C:/Users/Matt/Desktop/offense_Data.csv', 'a') as f:
...
for year in years:
    if year < 2015:
        url = 'http://espn.go.com/nfl/statistics/team/_/stat/total/year/' + str(year)
    elif year == 2015:
    ...
    for stat in stats:
        if year < 2015:
            tags.append(soup.findAll(href = re.compile('http://espn.go.com/nfl/statistics/team/_/stat/' + str(stat) + '/year/' + str(year))))
        elif year == 2015:
...
for url in tags:
    ...
    if url[0:57] == 'http://espn.go.com/nfl/statistics/team/_/stat/total/year/':
        year = url[57:61]
        ReformatDataFrame(year, df[0], 1, 1, FindStat(url))
    
    # If it is year 2015 (total offense statistics)
    elif url == 'http://espn.go.com/nfl/statistics/team/_/stat/total':
        year = 2015
        ReformatDataFrame(year, df[0], 1, 1, FindStat(url))
